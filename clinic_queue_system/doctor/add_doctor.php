...
if (isset($_POST['submit'])) {
    ...
    $check = mysqli_query($conn, "SELECT * FROM doctor WHERE name='$name' AND specialty='$specialty'");
    if (mysqli_num_rows($check) > 0) {
        echo "<script>alert('⚠️ This doctor already exists!'); window.history.back();</script>";
        exit();
    }
    // =============================
    
    $stmt = mysqli_prepare($conn, "INSERT INTO doctor (name, specialty, phone, room_number) VALUES (?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "ssss", $name, $specialty, $phone, $room);
    
    if (mysqli_stmt_execute($stmt)) {
        header("Location: view_doctors.php?success=1");
        exit();
    }
}
?>
