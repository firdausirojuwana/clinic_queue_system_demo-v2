...
if (isset($_POST['submit'])) {
    ...
    $check = mysqli_query($conn, "SELECT * FROM medicine WHERE medicine_name='$medicine_name' AND dosage='$dosage'");
    if (mysqli_num_rows($check) > 0) {
        echo "<script>alert('⚠️ This medicine already exists!'); window.history.back();</script>";
        exit();
    }
    
    $stmt = mysqli_prepare($conn, "INSERT INTO medicine (medicine_name, dosage, unit_price, stock_quantity) VALUES (?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "ssdi", $medicine_name, $dosage, $unit_price, $stock_quantity);
